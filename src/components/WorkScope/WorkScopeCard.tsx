    ...
    >
        ...
        {hasPhotos ? (
          <>
            ...
            <div className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-slate-950/85 backdrop-blur-md border border-white/15 text-white text-[10px] sm:text-xs font-mono uppercase tracking-wider font-semibold shadow-md">
            ...
            {photoCount > 1 && (
              <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10">
                <span className="inline-flex items-center gap-1 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-full bg-slate-950/85 backdrop-blur-md border border-white/15 text-slate-100 text-[10px] sm:text-xs font-mono shadow-md font-medium">
      ...
      <div className="flex flex-col flex-grow p-4 sm:p-6 lg:p-7 justify-between space-y-4 sm:space-y-5">
        <div className="space-y-2 sm:space-y-3">
          ...
          <p className="text-slate-600 font-light text-xs sm:text-sm lg:text-base leading-relaxed line-clamp-2 sm:line-clamp-3">
        ...
        <div className="space-y-3 pt-3 sm:pt-4 border-t border-slate-100">
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {item.tags.slice(0, 3).map((tag, idx) => (
              <span
                key={idx}
                className="px-2 py-0.5 sm:px-3 sm:py-1 rounded-md sm:rounded-lg text-[10px] sm:text-xs font-mono text-slate-600 bg-slate-100 group-hover:bg-orange-50 group-hover:text-orange-700 transition-colors border border-slate-200/60"
              >
                #{tag}
              </span>
            ))}
          ...
          <div className="w-full py-2.5 px-4 sm:py-3.5 sm:px-5 rounded-xl sm:rounded-2xl bg-slate-900 text-white font-bold text-xs sm:text-sm flex items-center justify-between group-hover:bg-orange-600 group-hover:shadow-lg transition-all duration-300">
            ...
            <div className="flex items-center gap-1">
              <span className="text-[11px] sm:text-xs font-mono opacity-80 group-hover:opacity-100">Galeria</span>
